...
function clearBoard(){
  document.getElementById('board-body').innerHTML =
    usernames.map((u,i)=>`<tr><td>${i+1}</td><td>${u}</td><td>—</td><td>—</td></tr>`).join('');
}
...
async function showDetails(user){
  ...
  const fields=[
    ['Username',p.username||'—'],['Name',p.name||'—'],['Title',p.title||'—'],
    ['Status',p.status||'—'],['Location',p.location||'—'],
    ['Country',p.country? p.country.split('/').pop():'—'],['Joined',j],
    ['Last Online',lo],['Followers',p.followers??'—']
  ];
  ...
  document.getElementById('player-details').innerHTML=
    `<div class="profile-header"><img src="${p.avatar||''}" alt="${user}"></div>
     <table class="details-table">
...
function initInspector(){
  ...
  cells.forEach(c=>c.addEventListener('click',async()=>{
    ...
    c.classList.add('selected'); clearDetails(); await showDetails(c.dataset.user);
  }));
...
function initToggles(){
  ...
  const btnT=document.getElementById('toggle-tournament');
  const btnO=document.getElementById('toggle-online');
  btnT.addEventListener('click',()=> container.classList.add('tournament'));
  btnO.addEventListener('click',()=> container.classList.remove('tournament'));
}
...
function initThree(){
  ...
  function resize(){ const w=window.innerWidth/2,h=window.innerHeight;
    renderer.setSize(w,h); camera.aspect=w/h; camera.updateProjectionMatrix(); }
  window.addEventListener('resize',resize); resize();
  const geo=new THREE.IcosahedronGeometry(1.2,0);
  const mat=new THREE.MeshBasicMaterial({color:0x00fffc,wireframe:true,wireframeLinewidth:8});
  const mesh=new THREE.Mesh(geo,mat); scene.add(mesh);
  (function animate(){ requestAnimationFrame(animate);
    mesh.rotation.x+=0.04; mesh.rotation.y+=0.03;
...
window.addEventListener('DOMContentLoaded',()=>{
  initControls(); updateBoard('rapid'); initInspector(); initToggles(); initThree();
});